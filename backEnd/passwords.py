...
class PassInfo:
    ...
    def get_password(self,client_email=None,session_key=None,server_public_key=None,client_permanent_key=None):
        ...
        if self._password:
            #full password already downloaded
            return self._password
        else:
            #doesn't exist in full, downloading...
    ...
    @abstractmethod
    def _get_password_full(self,client_email=None,session_key=None,
                           server_public_key=None,client_permanent_key=None):
        pass
    ...
    def delete_password(self,client_email,session_key,server_public_key):
        if self._manager == 1: 
            data = pr.delete_password(server_public_key=server_public_key,
                                      session_key=session_key,client_email=client_email,passID=self._passID)
            return data
        else:
            return "Not manager"
    ...
    def delete_password_instance(self,client_email,new_manager_email,session_key,server_public_key):
        data = pr.delete_password_instance(server_public_key=server_public_key,session_key=session_key,
                                           client_email=client_email,passID=self._passID,new_manager_email=new_manager_email)
        return data
        
    def set_to_lockdown(self,client_email,session_key,server_public_key):
        data = pr.set_to_lockdown(server_public_key=server_public_key,session_key=session_key,
                                  client_email=client_email,passID=self._passID)
    ...
    def add_manager(self,client_email,new_manager_email,session_key,server_public_key):
        if self._manager == 1:
            data = pr.add_manager(server_public_key=server_public_key,session_key=session_key,
                                  client_email=client_email,passID=self._passID,new_manager_email=new_manager_email)
            return data
        else:
            return "NOT MANAGER"     
    ...
    def get_password_users(self,client_email,session_key,server_public_key):
        if self._password_users:
            #check users aren't already downloaded, if they are return them
            return self._password_users
        else:
            if self._manager == 1:
                data = pr.get_password_users(server_public_key=server_public_key,session_key=session_key,
                                             client_email=client_email,passID=self._passID,manager_only=False)
    ...
    def get_password_managers(self,client_email,session_key,server_public_key):
        if self._password_managers:
            ...
            if self._manager == 1:
                data = pr.get_password_users(server_public_key=server_public_key,session_key=session_key,
                                             client_email=client_email,passID=self._passID,manager_only=True)
                if data not in ["NOT MANAGER","UNAUTHENTICATED","KEY EXPIRED","NO KEY"]:
        ...
        if self._manager == 1:
            if type == "Password":
                if self._password_key:
                    ...
                    data = pr.update_password(server_public_key=server_public_key,session_key=session_key,
                                              client_email=client_email,passID=self._passID,new_info=encrypted_password,type=type)
                else:
                ...
                data = pr.update_password(server_public_key=server_public_key,session_key=session_key,
                                          client_email=client_email,passID=self._passID,new_info=new_info,type=type)
            return data 
        else:
            return "NOT MANAGER"
    ...
    def share_password(self,client_email,session_key,server_public_key,recipient_userID,manager,client_permanent_key):
        ...
        if self._manager == 1:
            if self._password_key:
                public_key = pr.get_public_key(server_public_key=server_public_key,
                                               session_key=session_key,recipient_UserID=recipient_userID)
                ...
                self._get_password_full(client_email=client_email,session_key=session_key,
                                        server_public_key=server_public_key,client_permanent_key=client_permanent_key)
                return self.share_password(client_email,session_key,server_public_key,recipient_userID,
                                           manager,client_permanent_key)
        else:
            return "NOT MANAGER"
...
class Info(PassInfo):
    def __init__(self, passID, title, manager, password = None, password_key = None, 
                 password_users = None, password_managers = None):
    ...
    def _get_password_full(self,client_email=None,session_key=None,server_public_key=None,client_permanent_key=None):
        if self._password:
            ...
            data = pr.get_password(server_public_key=server_public_key,session_key=session_key,
                                   client_email=client_email,passID=self._passID)
...
class Password(PassInfo):
    def __init__(self, passID, title, url, username, manager, password = None, password_key = None, 
                 additional_info = None, password_users = None, password_managers = None):
        super().__init__(passID, title, manager, password, password_key, password_users, password_managers)
        self.__url = url
    ...
    def _get_password_full(self,client_email=None,session_key=None,
        ...
        if self._password:
            ...
            data = pr.get_password(server_public_key=server_public_key,
                                   session_key=session_key,client_email=client_email,passID=self._passID)
            if data in ["UNAUTHENTICATED","KEY EXPIRED","NO KEY","LOCKDOWN"]:
                return data
            else:
